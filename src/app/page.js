...
const BASE_URL = `http://${typeof window !== 'undefined' ? window.location.hostname : 'localhost'}:3000`;

const Card = ({ drone }) => {
  const handleRecharge = async () => {
    try {
      if (!drone.isInUse) {
        const response = await fetch(`${BASE_URL}/drone/recharge`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ uid: drone.uid }),
        });
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchDroneData = async () => {
      try {
        const response = await fetch(`${BASE_URL}/drone/data`);
